...
class PPOActorWithEstimator(nn.Module):
    def __init__(self, state_shape, action_shape):
        ...
        # Action branch
        self.action_head = nn.Linear(128, action_shape[0])
        self.log_stds = nn.Parameter(torch.zeros(1, action_shape[0]))
...
class PPO(Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device=torch.device('cuda'), seed=0,
                 ...
                 bool_soft_penalty=False#add a soft penalyt
                 ):
        super().__init__()
        ...
        self.lambd_l2 = 1e-4 #1e-5 : almost no constraint. 1e-4:middle constraint (reasonable). 1e-3 : strong constraint.
...
class SACActor(nn.Module):
    ...
    def __init__(self, state_shape, action_shape):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(128, 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, 2 * action_shape[0]), #mu and sigma.
        )
...
class SACActorWithEstimator(nn.Module):
    def __init__(self, state_shape, action_shape):
        ...
        self.action_head = nn.Linear(128, 2 * action_shape[0])  # mean and log_std
...
class SACCritic(nn.Module):
    ...
    def __init__(self, state_shape, action_shape):
        ...
        self.net1 = nn.Sequential(
            nn.Linear(state_shape[0] + action_shape[0], 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, 1),
...
class SAC_alpha(Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device=torch.device('cuda' if torch.cuda.is_available() else "cpu"), seed=0,
                 ...
                 lr_scheduler = None, lr_type="const",model_type="with_rope"):
        super().__init__()
    ...
    def update(self,num_step):
        ...
        #Mini-batch sampling.
        states, actions, rewards, dones, next_states,state_rope_middle,state_rope_middle_estimate = self.buffer.sample(self.batch_size)
