...
try:
    import utils._create_update_model_simple as update_model
except ImportError:
    print("Warning: Could not import utils._create_update_model_simple")
    update_model = None
...
class MPCController:
    def __init__(self, config_env=r"config/env/mpc.yaml", 
        ...
        self.weight_turn = float(self.config_env['params']['weight_turn'])
    ...
    def _create_cost_weight(self):
        ...
        """
        cost_weight = casadi.SX.zeros((30,30))
        cost_weight[0:3,0:3] = 0.25*casadi.SX.eye(3) * self.weight_turn
        cost_weight[0:3,6:9] = -0.5*casadi.SX.eye(3) * self.weight_turn
        cost_weight[0:3,12:15] = 0.25*casadi.SX.eye(3) * self.weight_turn
        cost_weight[6:9,0:3] = -0.5*casadi.SX.eye(3) * self.weight_turn
        cost_weight[6:9,6:9] = 1.0*casadi.SX.eye(3) * self.weight_turn
        cost_weight[6:9,12:15] = -0.5*casadi.SX.eye(3) * self.weight_turn
        cost_weight[12:15,0:3] = 0.25*casadi.SX.eye(3) * self.weight_turn
        cost_weight[12:15,6:9] = -0.5*casadi.SX.eye(3) * self.weight_turn
        cost_weight[12:15,12:15] = 0.25*casadi.SX.eye(3) * self.weight_turn
        cost_weight[21:24,27:30] = 0.5*casadi.SX.eye(3) 
        cost_weight[27:30,21:24] = 0.5*casadi.SX.eye(3) 
        
        cost_weight = -cost_weight # for minimization. 
        self.Q = cost_weight
        self.Q_f = cost_weight
        self.R = casadi.diag([0.01,0.01,0.01])
    ...
    def make_RK4(self):
        ...
        a_human = casadi.SX.sym("a_human", 3)

        f = update_model.create_update_model_4springs(
            k_s=self.k_s, l=self.unit_length, m=self.m, G=self.G, 
            ns=self.ns, nx=self.nx, nu=self.nu
        )

        r1 = f(x=states, u=ctrls, a_human=a_human)["x_dot"]

        r2 = f(x=states + self.dt * r1 / 2, u=ctrls, a_human=a_human)["x_dot"]

        r3 = f(x=states + self.dt * r2 / 2, u=ctrls, a_human=a_human)["x_dot"]

        r4 = f(x=states + self.dt * r3, u=ctrls, a_human=a_human)["x_dot"]

        states_next = states + self.dt * (r1 + 2 * r2 + 2 * r3 + r4) / 6

        RK4 = casadi.Function("RK4", [states, ctrls, a_human], [states_next], 
                             ["x", "u", "a_human"], ["x_next"])
    ...
    def make_integrator(self):
        ...
        ode = f(x=states, u=ctrls, a_human=a_human)["x_dot"]

        # Add ctrl_hum to the parameters so the integrator can use it

        dae = {"x": states, "p": casadi.vertcat(ctrls, a_human), "ode": ode}
    ...
    def compute_stage_cost(self, x, u):
        ...
        # Copy non-velocity parts
        for i in range(offset):
            x_normalized.append(x[i])
        
        # Normalize velocity parts
        for i in range(self.ns+1):
            vel_slice = x[offset+i*3:offset+i*3+3]
            norm_val = casadi.norm_2(vel_slice) + 1e-6
            normalized_vel = vel_slice / norm_val
            for j in range(3):
                x_normalized.append(normalized_vel[j])
        
        x_normalized = casadi.vertcat(*x_normalized)
        cost = (casadi.dot(self.Q @ x_normalized, x_normalized) + casadi.dot(self.R @ u, u)) / 2
        return cost
    ...
    def compute_terminal_cost(self, x):
        ...
        x_normalized = casadi.vertcat(*x_normalized)
        cost = casadi.dot(self.Q_f @ x_normalized, x_normalized) / 2
        return cost
    ...
    def make_qp_formulation(self):
        ...
        for k in range(self.K):
            ...
            eq = X[k + 1] - RK4(x=X[k], u=U[k], a_human=a_human)["x_next"]
            G.append(eq)
        J += self.compute_terminal_cost(X[-1])

        # Pass reference trajectory as parameters
        qp = {"x": casadi.vertcat(*X, *U), "p": a_human, "f": J, "g": casadi.vertcat(*G)}
        return qp
    ...
    def make_nlps(self):
        ...
        # Pass reference trajectory as parameters
        nlp = {"x": casadi.vertcat(*X, *U), "p": a_human, "f": J, "g": casadi.vertcat(*G)}
    ...
    def solve_optimization(self, x_init, a_human, x0=None, S=None):
        ...
        try:
            res = S(lbx=lbx, ubx=ubx, lbg=lbg, ubg=ubg, x0=x0, p=a_human)
            # Extract results
            x_sol = res["x"]
            ...
            print(f"Solving optimization with {len(x_init_list)} initial states")
    ...
    def compare_solvers(self, x_init):
