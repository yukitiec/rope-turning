...
class UR5eRopeEnv(gym.Env):
    def __init__(self, fps, step_episode, client_id):
        ...
        self._r_min_turn = 0.4  # lower bound for the human's turning radius.
        self._r_max_turn = 0.6  # upper bound for the human's turning radius.
        self._zmin_rob = (
    ...
    def update_target_position(self, dt):
        ...
        if (
            self.bool_eval == False and self.n_circuit % self._n_circuit_update == 0
        ...
        if self.bool_base:  # baseline model.
            self.target_position = self.rosbag["target_position"][
                self.counter_step % self.rosbag["target_position"].shape[0]
            ]
            self.counter_step += 1
        else:
            self.rosbag["target_position"] = np.vstack(
                (self.rosbag["target_position"], np.array([self.target_position]))
            )
    ...
    def step(self, action, bool_base=False, bool_render=False):
        ...
        #action_imitate = self._update_target_sync(obs=obs, action=action)
        #for i, (p_delta, p_imitate) in enumerate(
        #    zip(action, action_imitate)
        #):  # residual target.
        #    if isinstance(
        #        p_delta, (list, np.ndarray)
        #    ):  # Check if vel is a list or array
        #        p_delta = p_delta[0]  # Take the first element if it is
        #    if i < 3:
        #        p_target = p_imitate + p_delta  # p_target = p_imitate + p_delta
        ...
        vel_eef = np.zeros(6)
        if np.linalg.norm(action[0:3]) > 2.0: #max speed is 2.0 m/s
            action[0:3] = action[0:3] / np.linalg.norm(action[0:3]) * 2.0
        vel_eef[0:3] = action# pass linear velocity to the eef.
        ...
        vel_rope_middle = vel_rope_middle
        ...
        return obs, state_rope_middle, reward, done, {}
    ...
    def _get_observation(self):
        ...
        for i, link_id in enumerate(self.rope_link_ids):
            ...
            if (
                r_magnitude > self.link_length * 500
    ...
    def reset(self, i=0, rope_length=0.5, bool_base=False, bool_eval=False):
        ...
        if bool_eval:
            self.moving_point_radius = 0.3 * self.rope_length
        else:
            while (
                zmin <= self._zmin_rob or self._zmax_rob <= zmax
            ):  # not meet the criteia.
                self.moving_point_radius = np.random.uniform(
                    self._r_min_turn * self.rope_length,
                    self._r_max_turn * self.rope_length,
                )  # get the randomized turning radius.
